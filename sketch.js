...
function draw() {
  ...
  // 產生並更新火焰特效粒子
  for (let i = 0; i < 5; i++) {
    particles.push(new Particle(circleX, circleY));
  }
  ...
  text("411730608 郭碧能", width / 2, height - 10);
}
...
class Particle {
  constructor(x, y) {
    this.x = x;
    this.y = y;
    this.vx = random(-2, 2);      // X 軸稍微隨機飄動
    this.vy = random(-6, -1);     // Y 軸向上移動
    this.alpha = 255;             // 初始透明度
    this.size = random(20, 50);   // 初始大小
  }

  update() {
    this.x += this.vx;
    this.y += this.vy;
    this.alpha -= 5;              // 每一幀逐漸變透明
    this.size -= 0.5;             // 每一幀逐漸縮小
  }

  show() {
    noStroke();
    // 利用透明度來產生顏色的漸變，剛產生時偏黃，消失時偏深紅
    fill(255, map(this.alpha, 0, 255, 0, 150), 0, this.alpha);
